...
from .vox_api import VoxLibraryCache, VoxLibraryDetails
from .action import XbmcAction, XbmcActionFactory
...
def login():
    ...
    if not(os.path.exists(vox_cloud_bearer_token)) or older_than(vox_cloud_bearer_token, days=0.1):
        username = addon.getSetting('facebook_username')
        password = addon.getSetting('facebook_password')
        xbmc.log(f"Will attempt to login to Facebook with username: '{username}'", xbmc.LOGINFO)
        vox_oauth_config = config['oauth']
        client_id, client_secret = vox_oauth_config['client_id'], vox_oauth_config['client_secret']
        oauth_login = VoxOAuthLogin(FacebookOauthProvider(vox_oauth_config['facebook']), client_id, client_secret)
        oauth_login.login(username, password)
        xbmcgui.Dialog().notification("Login", f"Facebook login with username {username} succeeded.", icon=xbmcgui.NOTIFICATION_INFO)        
...
def create_caches(session, vox_cloud_bearer_token):
    def _create_and_store_cache():
        ...
        cache.to_file(albums_file=vox_cloud_albums_db, artists_file=vox_cloud_artists_db, tracks_file=vox_cloud_tracks_db)
        return cache

    def _is_stale_cache():
        date_fmt = "%Y-%m-%d %H:%M:%S"
        cache_mtime = datetime.datetime.fromtimestamp(os.path.getmtime(vox_cloud_albums_db)).astimezone()
        ...
        xbmc.log(f"""Cache created on {formatted_cache_time}.  VOX Library last modified on: {formatted_vox_time}""", xbmc.LOGINFO)
    ...
    cache = None
    file_caches_exist = os.path.exists(vox_cloud_artists_db) and os.path.exists(vox_cloud_albums_db) and os.path.exists(vox_cloud_tracks_db)
    if not(file_caches_exist) or _is_stale_cache():
        ...
        cache = VoxLibraryCache.from_file(session, vox_cloud_bearer_token, 
                                        albums_file=vox_cloud_albums_db, artists_file=vox_cloud_artists_db, 
                                        tracks_file=vox_cloud_tracks_db)
    return cache
    #xbmc.log(f"Created a VOX Cloud database: {vox_cloud_db}", xbmc.LOGINFO)
...
class Router(viewtype):
    def __init__(self, args):
        ...
        xbmc.log(f"Successfully created Router class for addon: {addon_name} with args: {args}, bearer token: {vox_cloud_bearer_token}", xbmc.LOGINFO)
