...
class FacebookOauthProvider(OAuthProvider):
    ...
    def __get_facebook_login_form(self, headers: dict, fb_url: str) -> dict:
        ...
        html_form = dict(
            url=urljoin(fb_url, html_login_form.attrib["action"]),
            form_fields={
    ...
    def __get_oauth_response(
        self, session: requests.Session
    ) -> None:  # , ext: str, hash: str):
        ...
        oauth_params = dict(
            auth_type="rerequest",
            response_type="code",
            redirect_uri=vox_redirect_url,
            scope="email",
            client_id=vox_api_key,
            ret="login",
            fbapp_pres=0,
            tp="unspecified",
        ...
        oauth_resp = self.session.get(
            oauth_url, params=oauth_params, headers=self.fb_login_config["headers"]
        )
